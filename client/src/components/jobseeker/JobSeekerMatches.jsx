...
const JobSeekerMatches = () => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        const res = await axios.get('/api/matches/jobseeker');
        setMatches(res.data);
        setLoading(false);
      } catch (err) {
        console.error(err);
  ...
  const handleApply = async (matchId) => {
    try {
      await axios.put(`/api/matches/${matchId}/status`, { status: 'applied' });
      setMatches(matches.map(match => 
        match._id === matchId ? { ...match, status: 'applied' } : match
      ));
      setAlert('Application submitted successfully', 'success');
    } catch (err) {
      console.error(err);
      setAlert('Error submitting application', 'error');
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Job Matches</h1>
      ...
      {filteredMatches.length > 0 ? (
          ...
          {filteredMatches.map(match => (
            <div key={match._id} className="bg-white rounded-lg shadow-md p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-xl font-bold text-gray-800">{match.job.title}</h2>
              ...
              <div className="mb-4">
                ...
                <p className="text-gray-600 text-sm">{match.job.description}</p>
                <div className="mt-2">
                  <span className="inline-block bg-gray-100 text-gray-800 px-2 py-1 rounded text-xs mr-2">
                    {match.job.jobType}
                  </span>
                  {match.job.salary && (
                    <span className="inline-block bg-gray-100 text-gray-800 px-2 py-1 rounded text-xs">
                      ${match.job.salary.toLocaleString()}/year
                    </span>
                  )}
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <p className="text-gray-600">
            {filter === 'all' 
              ? 'Complete your profile to get matched with suitable jobs.'
